   ...
   // const navigate = useNavigate();
    ...
    useEffect(()=>{
        ...
        async function fetchTask(){
            const docRef = doc(db, "TaskList", params.id)
    ...
    <div className="bg-white mx-auto flex justify-center items-center flex-col max-w-6xl p-4 rounded-lg shadow-lg mt-2">
      <div className="mx-auto flex-1 bg-blue-300 w-full h-[400px] lg-[400px] rounded-lg">
         ...
         <p className="flex ml-6 text-2xl font-bold mb-3 text-blue-800">
             {task.priority === 'high' ? <FcHighPriority className=" mr-1 sm:text-4xl" /> : null}
             {task.priority === 'low' ? <FcLowPriority className=" mr-1 sm:text-4xl" /> : null}
             {task.priority === 'Medium' ? <FcMediumPriority className=" mr-1 sm:text-4xl" /> : null}
             Priority:
            <span className={`ml-1 ${task.priority === 'low' ? 'text-green-500' : task.priority === 'normal' ? 'text-blue-500' : 'text-red-500'}`}>{task.priority}</span> 
        </p>
